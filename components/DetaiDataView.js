...
const DTable = props => {
  ...
  const transfrom = () => {
    if (props.type == '1') {
      return props.data.vervar.map(vvar => {
        let v = props.data.turtahun.map(e => {
          ...
          return props.data.datacontent[k]
            ? renderValue(String(props.data.datacontent[k]), props.v.unit)
            : '-';
        });
        // console.log('type 1 v : ', v)
        // console.log(vvar.label)
        return [
          vvar.label.length > 20 ? renderCapitalOnly(vvar.label) : vvar.label,
          ...v,
        ];
      });
    } else {
  ...
  const header = () => {
    return props.type == '1'
      ? [
          props.data.labelvervar,
          props.data.turvar.length > 1 ? props.turvar.label : props.v.title,
        ]
      : [
          'Tahun',
          ...(props.data.turtahun.length > 1
            ? props.data.turtahun.map(e => e.label)
            : [props.v.title]),
        ];
  };
  ...
  const minWidth = Dimensions.get('window').width - 20;
        ...
        <ScrollView>
          <View>
            ...
            <View
              ...
              {props.data.vervar.map((e, i) => (
                <Caption key={`vervar-${e.val}-${e.label}-${i}`}>
...
const TabelView = props => {
  ...
  return loaded ? (
    <View
      ...
      {type == '111' ? (
        <Title style={titleTableStyle}>
          {v.title} {turvar.label}
          {data.labelvervar == turth.label ? '' : ` Dalam ${turth.label}`}{' '}
          {turth.label} {tahun.label}
        </Title>
      ) : type == '101' ? (
        <Title style={titleTableStyle}>
          {v.title}
          {data.labelvervar == turth.label
            ? ''
            : ` Dalam ${turth.label}`} Tahun {tahun.label} ( {turvar.label} )
        </Title>
      ) : type == '011' ? (
        <Title style={titleTableStyle}>
          {v.title}
        ...
        <Title style={titleTableStyle}>
          ...
          {data.labelvervar == turth.label
            ? ''
            : ` Dalam ${turth.label}`} Tahun {tahun.label}
        </Title>
      ) : type == '110' || type == '100' ? (
        <Title style={titleTableStyle}>
          {v.title} {turvar.label} {data.vervar[0].label}
        </Title>
      ) : (
        <Title style={titleTableStyle}>
          {v.title} {data.vervar[0].label}
        </Title>
      )}
      {type[2] == '1' ? (
        <DTable
...
const DetaiDataView = props => {
  ...
  return loaded && !refreshing && !netErr ? (
    <ScrollView
      contentContainerStyle={{
        flex: 1,
      }}>
      <TabelView data={data} variable={props.variable} />
    </ScrollView>
    ...
    <ScrollView
      ...
      <Title>Koneksi Terputus Silahkan Coba Kembali</Title>
    </ScrollView>
  ) : (
    //
    <ScrollView
      ...
      <View
        ...
        {data.var_id ? (
          ...
          <Title>Silahkan pilih variabel data</Title>
        )}
      </View>
    </ScrollView>
  );
